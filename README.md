...
- 사용자에게 유용한 데이터를 제공하기 위해서 네트워크에 대한 기본 지식은 필수였습니다. 그러던 중, 실제로 정보를 제공하기 위해 사용하는 네트워크 유틸리티를 직접 구현하면 더 깊은 네트워크 개념과 원리를 알 수 있다는 생각을 해보았습니다. Node.js는 저수준의 네트워크 정보를 가져올 수 없기 때문에 완전히 라이브러리 없이 구현하기는 힘들었습니다. 하지만 최소한의 도움으로 유틸리티를 직접 구현하고자 하였습니다.

## 2. 네트워크 기본 개념 공부하기

- 네트워크 유틸리티를 라이브러리를 최소화하여 구현하기 위해서는 우선 만들 때 사용하는 네트워크 개념을 알아야 했습니다.
