...
- [🔥 Challenges](#-challenges)
  ...
  - [2. 네트워크 기본 개념 공부하기](#2-네트워크-기본-개념-공부하기)
  - [3. `traceroute` 직접 구현하기](#3-traceroute-직접-구현하기)
...
- 사용자에게 네트워크 정보를 제공하기 위해 기본 지식을 공부하였습니다. 네트워크 유틸리티를 사용하여 정보를 가져오던 중에 실제로 구현해보면 네트워크의 개념과 원리를 더 깊이 알 수 있다는 생각이 들었습니다.
- 두 인터넷 연결 간의 경로를 알기 위해 사용했던 `traceroute`와 네트워크 연결 상태와 속도 정보를 가져오기 위한 `ping` 유틸리티를 직접 구현해보기로 하였습니다.
- 라이브러리를 사용하지 않고 구현해 보려고 하였지만 Node.js가 가지는 한계점이 있었습니다. 그 한계점은 저수준의 네트워크 정보를 가져올 수 없다는 것이었습니다. C++을 통해 직접 구현해볼 수 있었지만 시간적 한계로 인해 일부 라이브러리를 사용하였습니다.

## 2. 네트워크 기본 개념 공부하기

- 네트워크 유틸리티를 직접 구현하기 위해 필요한 기본 개념은 다음과 같았습니다:

  - UDP(User Datagram Protocol)

    - UDP는 시간에 민감한 데이터를 전송하기 위해 사용되는 프로토콜입니다. 예를 들어 비디오 재생 또는 DNS 조회와 은 곳에서 사용됩니다.
