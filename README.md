...
- [⚒️ 기술 스택](#️-기술-스택)
  - [❓ Why React? (React vs. Next.js)](#-why-react-react-vs-nextjs)
    - [◉ 사용자에게 더 나은 서비스를 제공할 수 있지 않을까?](#-배경)
    - [◉ 가정: SSR이 CSR보다 렌더링 속도가 빠를 것이다.](#-가정)
    - [◉ React vs. Next.js](#-테스트)
    - [◉ SSR과 CSR의 렌더링 속도가 큰 차이를 보이지 않다.](#-결과)
- [⚡️ 사전 준비](#️-사전-준비)
  - [◉ 기획](#-기획-왜-그-많은-주제-중에-네트워크)
  - [◉ 검증: Proof of Concept(PoC)](#-검증-proof-of-conceptpoc)
  - [◉ 느낀점: 기획의 중요성](#-느낀점-기획의-중요성)
- [🏃‍➡️ 과정](#️-과정)
  - [1. 유저에게 어떤 정보를 제공할 것인가](#1-유저에게-어떤-정보를-제공할-것인가)
    - [1-1. 네트워크 기본 개념을 공부하자!](#1-1-계획-네크워크-기본-개념을-공부하자)
    - [1-2. 네트워크 개념을 정리하다.](#1-2-실행-개념을-정리하다)
    ...
    - [1-3. "연결이 왜 안될까?"에 대한 정보를 제공하다.](#1-3-결과-연결이-왜-안될까에-대한-정보를-제공하다)
    - [1-4. 연결이 왜 안되는지가 아닌 주어진 URL의 네트워크 정보를 제공하자!](#1-4-수정-연결이-왜-안되는지가-아닌-주어진-url의-네트워크-정보를-제공하자)
    - [1-5. 다양한 지리적 위치에서의 네트워크 정보를 제공하다.](#1-5-발전-다양한-지리적-위치에서의-네트워크-정보를-제공하다)
    - [1-6. 기능의 부족한 점과 더 개선할 방법은 없을까?](#1-6-회고-기능의-부족한-점과-더-개선할-방법은-없을까)
  - [2. 다른 유저에게 정보를 어떻게 제공할 것인가](#2-다른-유저에게-정보를-어떻게-제공할-것인가)
    - [2-1. 이메일로 정보를 전달하자.](#2-1-계획-이메일로-정보를-전달하자)
    - [2-2. `nodemailer`를 이용한 메일 공유](#2-2-실행-nodemailer를-이용한-메일-공유)
...
- [🔥 도전](#-도전)
  ...
  - [구현 1: traceroute 직접 구현하기.](#-구현-1-traceroute-직접-구현하기)
    - [1-1. traceroute에 대해 공부하자.](#1-1-계획-traceroute에-대해-공부하자)
    - [1-2. 일부 라이브러리를 사용하여 traceroute를 구현하다.](#1-2-실행-일부-라이브러리를-사용하여-traceroute-구현)
    - [1-3. 지리적 위치 정보를 추적하다.](#1-3-결과-지리적-위치-정보를-추적하다)
    - [1-4. 회고](#1-4-회고)
  - [구현 2: ping 직접 구현하기.](#구현-2-ping-직접-구현하기)
    - [2-1. ping에 대해 공부하자.](#2-1-계획-ping에-대해-공부하자)
    - [2-2. ping 구현](#2-2-실행-ping-구현)
    - [2-3. 패킷 손실율을 계산하다.](#2-3-결과-패킷-손실율을-계산하다)
    - [2-4. 회고](#2-4-회고)
...
- 우선 사용자에게 무엇을 제공할 지 정하였습니다. "왜 특정 웹사이트에 접속이 불가능 할까?"라는 주제를 생각하며 PoC를 진행하였습니다. 해당 주제에 대해서는 네트워크 연결 문제, 웹사이트 서버 문제, DNS 문제, URL 입력 오류로 크게 4가지 소주제로 나누었습니다.
  ...
  - 네트워크 연결 문제는 라우터 문제, ISP 문제, 로컬 네트워크 문제, 연결 지연, 패킷 손실률, 비정상적인 트래픽 패턴 등이 있을 것이라고 생각하였습니다. 각각의 문제를 어떻게 구현할 것인지를 검증해 본 결과, ISP 경우에는 접근할 수 있는 권한이 없었기에 진행할 수 없었으며 로컬 네트워크 문제는 범위가 너무 넓어 구현하지 못할 것으로 판단하였습니다.
    그에 따른 라우터 문제, ISP 문제, 로컬 네트워크 문제, 연결 지연 문제 등을 생각하였지만 Node.js가 가지는 기능적인 한계인 저수준의 네트워크 정보는 가져오지 못하는 문제 때문에 ISP와 로컬 네트워크 문제는 제외시켰습니다. 그 외의 다른 문제들은 Node.js에서 제공하는 기본적인 모듈을 통해 구현할 수 있을 것이라 판단되었고, 외부 라이브러리를 사용하면 그 정보를 쉽게 얻을 수 있다는 사실을 알게 되었습니다. 예를 들어 라우터 문제의 경우에는 `pcap` 라이브러리를 사용하면 가져올 수 있었습니다.
...
- 네트워크 기본 개념들을 다음과 같이 정리하였습니다:
  ...
  - #### 1-2-4. TTL(Time-to-Live)
    ...
    - 홉이란 한 라우터와 다른 라우터 간의 여정을 말한다. 각 홉에서 라우터는 패킷의 TTL 값을 1 감소시킨다. TTL 값이 0이 되면 라우터는 더 이상 패킷을 전달하지 않고, "Time Exceeded" 메시지를 송신자에게 보내고 패킷을 페기한다.
...
### 1-6. 회고: 기능의 부족한 점과 더 개선할 방법은 없을까?
