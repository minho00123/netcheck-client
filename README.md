...
- [⚡️ 사전 준비](#️-사전-준비)
  ...
  - [◉ Proof of Concept(PoC)](#-proof-of-conceptpoc)
...
- [🏃‍➡️ 과정](#️-과정)
  - [1. 유저에게 어떤 정보를 제공할 것인가](#1-유저에게-어떤-정보를-제공할-것인가)
    ...
    - [1-6. 회고: 기능의 부족한 점과 더 개선할 방법은 없을까?](#1-6-회고-기능의-부족한-점과-더-개선할-방법은-없을까)
...
- 개인 프로젝트를 만들기 위해 다양한 아이디어를 생각해보았습니다. Rust 플레이그라운드를 만들어 웹페이지에서 실행해보는 프로그램을 만들어 보고 싶었으나, 제한된 시간 안에 완성해야 했기에 Rust에 대한 기본 지식이 없는 저는 다른 아이디어를 생각해야 했습니다. 하나의 데이터로 여러 종류의 계획서로 변환해주는 프로젝트도 생각해보았지만 혼자서 구현하기에는 주제가 너무 광범위하기에 제외하였습니다. 프로젝트를 만들면서 한층 더 저를 성장할 수 있게 해주고, 사용자를 만족시킬 수 있는 것이 무엇이 있을까 고민하던 중에 네트워크를 생각하게 되었습니다.

  - [아이디어 노트 노션 링크](https://dune-hammer-c89.notion.site/58dc7828f4274c9a80130fe5aa271351?pvs=4)
    경로를 알기 위해서는 다양한 네트워크 정보들이 필요했습니다. DNS, 보안, 그리고 연결이 제대로 되었는 지를 알아야 했습니다. 이러한 정보들을 한 곳에서 다른 사람들에게 제공할 수 있으면 좋겠다는 생각을 통해 프로젝트를 진행하였습니다.

## ◉ 왜 그 많은 주제 중에 네트워크?

- 현재 우리는 인터넷 없이 살 수 없는 상황이 되었습니다. 언제, 어디서든 인터넷을 이용해 많은 정보를 얻고 있습니다.아침에 일어나 날씨를 확인하고, 궁금한 단어가 있으면 핸드폰을 통해 찾아보고, 또한 실시간으로 영상통화를 진행하기도 합니다. 이러한 인터넷이 어떠한 경로를 통해 우리에게 전달되는지가 저의 가장 큰 궁금증이었습니다. 네트워크 관련 주제는 저의 네트워크 지식을 향상시키기에 좋은 경험이라고 생각했고, 상대방에게도 좋은 정보를 제공할 수 있는 기회라고 생각하였습니다.

## ◉ Proof of Concept(PoC)

- 우선 사용자에게 무엇을 제공할 지 정하였습니다. "왜 특정 웹사이트에 접속이 불가능 할까?"라는 주제를 생각하며 PoC를 진행하였습니다. 해당 주제에 대해서는 네트워크 연결 문제, 웹사이트 서버 문제, DNS 문제, URL 입력 오류로 크게 4가지 소주제로 나누었습니다. 가장 중요한 주제는 네트워크 연결 문제라고 생각했습니다. 그에 따른 라우터 문제, ISP 문제, 로컬 네트워크 문제, 연결 지연 문제 등을 생각하였지만 Node.js가 가지는 기능적인 한계인 저수준의 네트워크 정보는 가져오지 못하는 문제 때문에 ISP와 로컬 네트워크 문제는 제외시켰습니다.
- 다른 정보들은 각각의 효율적인 라이브러리를 찾아서 정보를 제공할 수 있다는 결론을 내렸습니다. 많은 라이브러리를 사용한다는 점에서 의존성이 높다는 생각이 들어 추후에는 직접 라이브러리를 구현해보자는 생각을 하게 되었습니다.
